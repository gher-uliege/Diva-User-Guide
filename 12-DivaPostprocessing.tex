...
Once you have downloaded it, just unzip the folder in the location of your choice (provided it is located on the path of your system). The \gnuplot\, window is activated either by typing \texttt{wgnuplot} in the Cygwin shell, or by creating a short-cut on your desktop to the executable \texttt{wgnuplot.exe}
...
Compile the Fortran sources (\texttt{forgnuplot$\cdots$ .f}) if necessary and place the executables into \texttt{./diva-\divaversion/bin.}

In directory divastripped/gnuwork/, edit \texttt{divaplot} so that the \texttt{gplot} indicates the correct path to your \gnuplot\, executable.
\example\\
\begin{verbatim}
#====================================================
...
From \texttt{divastripped}, after running an analysis, type \texttt{divagnu}: this will create the figures in directory \texttt{gnuwork/plots/}.
...
In old \diva\, versions, they are separated into two subdirectories: \texttt{CL}, for the \matlab\, programs adapted to the command line version, and \texttt{GUI}, for the graphical interface.
...
\diva\, provides analyzed and error fields under the NetCDF (network Common Data Form) format. It is an machine-independent format to represent scientific data. For more details, please consult \url{http://www.unidata.ucar.edu/software/netcdf/}.
...
\item download \textsl{Ncview} and unzip the folder
\item type\\ 
\textcom{
./configure\\
make\\
