...
\begin{description}
\item[divaanom:] computes the difference between the data and the provided reference field. 
\item[divabestguess:] provides estimates for the correlation length and the signal-to-noise ratio with methods chosen according to the number od data.
\item[divabigtest:] performs a test with a very large number of data points.
\item[divacalc:] performs the \diva analysis.

\item[divacck:] check the consistency of the initial contour file.
\item[divaclean:] cleans up the working directories by removing \file{fort.*} files from \directory{divawork} and \directory{meshgenwork}, as well as output files from \directory{output}.
\item[divacoa2cont:] converting ODV-format coastlines to \diva-format coastlines.
\item[divacont:] creates the coutours from a given bathymetry and a selected depth levels.
\item[divacont2grid :]
\item[divacpme :]
\item[divacv:]
\item[divacvclasses :]
...
\item[divaseminorm:]
\item[divasnbygrid:] 
\item[divasumup:] performs the last step of a semi-normed analysis: the sum of background field and analysed anomaly field.
\end{description}
...
The global correlation length \index{Correlation length} used for the analysis (see Section~\ref{sec:parammeaning} for the physical meaning). It has to be defined as a real positive number. 
