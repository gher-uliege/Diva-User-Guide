...
Outputs resulting from a \command{divadoall} run for input data preparation actions are placed in the \directory{input} directory for data sets extraction and in a \directory{newinput} subdirectory for the other actions as shown in the following table:
...
\hline
    &   \\
Data cleaning on       & \\
 mesh, outliers        &   A {\directory{newinput/divaparam}} subdirectory \\
...
\item {\bf Cleaning data and Relative Length}: Possible flag values are $0$, $1$, $2$, $3$, $4$, $5$, $6$ and $7$:
		...
		\item[*] $=7$ if data files are to be cleaned and relative length files depending on the gradient of depth to be generated.
              \end{itemize}
