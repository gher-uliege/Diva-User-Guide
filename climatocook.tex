...
\begin{enumerate}
\item
For easy use, import your data into Ocean Data View (ODV, found at \url{http://odv.awi.de/}). You can for example extract data from the world ocean atlas at \url{http://www.nodc.noaa.gov/OC5/SELECT/dbsearch/dbsearch.html} and choose the export in the native world ocean atlas. This allows a direct import of the file into ODV.
...
{\bf NOTE}: do not forget to deal adequately with data elimination based on quality flags, either before importing in ODV, selection within ODV, during export from ODV or when using {\tt divaselectorODV4} with file {\tt qflag}. Make sure  you export the quality flags with the data and you know their meaning.
...
\item Decide on your coordinate change {\tt ireg}
\end{enumerate}
...
\end{enumerate}
If it was not successful, check for implementation errors (be careful about end-of-line problems (remember {\tt dos2unix}), file permissions, limited disk space, input-data file formats, too fine coastlines leading to memory problems etc). In doubt, retry the example provided with the distribution to see if the problems stems from your data and implementation or the software itself.
...
\item Use a large correlation length 
...
\item Perform final analysis with full error calculation
