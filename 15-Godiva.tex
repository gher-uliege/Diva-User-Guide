...
The working directory to performs 4D-analyses is: \directory{GODIVA\_mm\_yyyy/JRAx/Climatology}.
...
All \diva shell scripts files for climatologies (or 4D-analyses) production are located in \par \directory{GODIVA\_mm\_yyyy/JRAx/Climatology}. This directory has its proper subdirectories \directory{input} where input data and files are placed, and \directory{output} where \diva outputs are stored.
...
\begin{itemize}
\item Extraction of data at selected depth levels.
...
\item Elimination of outliers from data sets.
