...
The working directory to performs 4D-analyses is : \underline{\directory{GODIVA\_2013/JRAx/Climatology}}.
...
\section{\diva 4D Climatology performence}
...
The main shell script file for climatologies performence is \file{divadoall}. The actions that one can perfom when running \file{divadoall} are:
...
\item analysis using a ``filtered'' mean backgoung field: all levels mean are first calculated an vertically filtered before being used  3D analyses.
\end{itemize}
...
\item[*] When parameters optimisation and/or analysis is activated, the execution is made for the levels between the values choosen for the lower and upper level numbers and takes into account the bound values (maximum and minimum) prescribed in the \file{driver}.
\end{itemize}
...
\begin{itemize}
\item Data set foles extraction.
...
To performe an action, one has to configure the \file{driver} file and give in the corresponding flag values and run the \file{divadoall} files script. For each action, a specific inputs are needed:
...
Outputs resulting from a \file{divadoall} run for input data preparation actions are placed in the \directory{input} for data sets extraction and in a \directory{newinput} subdirectory for the other actions as chown in the following table:
...
Parameters optimisation & A {\directory{newinput/divaparam}} subdirectory which contains \\
 ($L$ and $S/N$)        & \file {\tt param.par.var.$100xx$} files and summary files of the \\
...
\underline{\bf The sell script file \file{divadocommit}}: In order to be able to use the outputs of input data preparation actions, they must be copied to the \directory{input} directoty. This can be done by runnig the sell script file \file{divadocommit}. 
...
\begin{tabular}{|l|}
\hline
\file{divadocommit} replaces input files in \directory{input} directory by the ones \\
 found in   \directory{newinput} directory assuming that the \file{driver}, \file{varlist},\\
  \file{yearlist} and \file{monthlist} files are the ones used by \file{divadoall}\\
  to creat the \directory{newinput} subdirectory on which  \file{divadocommit} is run.\\
\hline
\end{tabular}
}
\vspace{-0.4cm}
\begin{itemize}
\item[*] When reference fields are generated, they are copied by the script file \file{divadocommit} in a subdirectory \directory{input/divarefe\_all}.
\end{itemize}
...
{\bf Note:} These options are automatically acctivated when the appropriat input data are provided.


 More options available in \diva\ 4D are:

\begin{itemize}
\item analyses using vertically filtered mean bckgroung
\item analyses with data transformation:  $\log$(data)-$\exp$(analysis), and anamorphosis transformations or user defined transfomation
...
{\bf Note:} These options are acctivated with a specific falg values in the \file{driver}.
...
\file{monthlist} & \\
\hline 
\file{constandrefe} & input file to acctivate advection \\
                    & constraint and/or reference fileds \\
\hline 
\end{tabular}
}
...
\file{NCDFinfo}       & Info file containing metadata for NetCDF files (see exaple \ref{NCDFinfoexp})\\
\hline
...
Comment
'This is only for DIVA developpement and texting work'
...
\subsubsection{Using advection constraint and/or reference field }
 ...
 The advection constraint and/or reference fields usage actions are acctivated by the corresponding flag values in \file{constandrefe} file (see example \ref{expconsreff}).
The advection constraint option is acctivated when the corresponding flag value is equal to $1$. 
The use of reference fields option is acctivated when the corresponding flag value is equal to $1$, in this case a year period  and month period codes must be provided in the corresponding lines.
...
\begin{itemize}
\item {\bf Data extraction}: Possible flag values: $0$,$1$,$-1$ and $-10$. If you activate the data extraction (flag value $\neq 0$) in the {\tt driver} file, the execution of {\tt divadoall} will run the {\tt divaselectorODV4} automatically, including interpolation to the levels specified in {\tt contour.depth}. Data will be extracted from the ODV spreadsheat file(s) specified in {\tt datasource}. {\tt divaselectorODV4} will recognises if the data export to ODV file was done with depths in ``meters'' or it was done with pressure (in dbar) vertical coordinate, you can either choose to map it as if they were meters or apply the Saunders correction. Choose flag $=-1$ to use pressure coordinate and assume they are meters, and flag value $=-10$ to use pressure coordinates and transform to meters by using the Saunders approach.\par
 If there is a {\tt qflist} file, the selection with {\tt divaselectorODV4} will only use those measurements for which the quality flag is one of those found in the file {\tt qflist}. In the absence of {\tt qflist}, no quality flag analysis is done and all data taken.\par
...
\item {\bf Boundary lines and coastlines generation}: Possible flag values are $0$, $1$,  $2$ and  $3$. When this action is activated (flag $\geq 1$), you must provide in the {\tt input} directory the files {\tt TopoInfo.dat} and  {\tt topo.grd} in addition to {\tt contour.depth} file.
              \begin{itemize}
...
\item {\bf Analysis}: analysis and reference fields can be performed in different ways:
  ...
  \item  {\bf Perform analysis}: Possible flag values are $1$, $10$, $11$, $12$, $13$ and $14$:
              \begin{itemize}
                \item[*] $=1$ if analysis fields of the given variable are to be performed for all the layers between $L_1$ and $L_2$ which are the flag values for {\it lowerlevel number} and {\it upperlevel number} in the {\tt driver}.
                ...
                \item[*] $=14$ if analysis fields of the given variable are to be performed with user choosen transformation function.
              \end{itemize}
   \item  {\bf Perform reference fields}: Possible flag values are $2$, $20$, $21$, $22$, $23$ and $24$:
              \begin{itemize}
                \item[*] $=2$ if semi normed reference fields of the given variables (prescribed in {\tt varlist} and for time periods described in {\tt yearlist} and {\tt monthlist}) are to be performed for all the layers between $L_1$ and $L_2$, which are the flag values for {\it lowerlevel number} and {\it upperlevel number} in the {\tt driver}.
                ...
                \item[*] $=24$ if reference fields of the given variable are to be performed with user choosen transformation function.
              \end{itemize}
...
\item[*] {\tt plotboundingbox.dat}: contains the box for plotting. This is typically used to plot only the region of interest, without overlapping regions with other climatologies (the numerical fields include the overlapping regions, only the plotting is limited with the {\tt plotboundingbox.dat} file).
\end{itemize}
{\bf Note}: the gnuplot colorbars use a scale that is actually remapped to the bounds found in {\tt VAR.bound}. Exemple:
if your colorbar definition goes from 0 to 10 and the {\tt VAR} bounds are from 0 and 100, a value of 50 in the variable analyzed will use the color found in the colorbar definition at value 5. To help you designing a specially adapted colorbar lets say for salinity, it is therefore a good idea to define the colorbar with the same bounds as those in {\tt VAR.bounds}.\par
{\bf Note}: for adapting the color palette, file {\tt gnuplotcolornames} contains a list of preexisting colors and their hexadecimal codes you can use instead of names.
...
{\bf Note}: If you use {\tt divadoall} (or {\tt divaselectorODV4}) to extract data and create data input files, columns 5, 6 ,7 and 8 contain respectively groups years, month, days and hours (1 for the first year in the selection etc).
\end{itemize}
