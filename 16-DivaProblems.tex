...
You can activate an iterative solver by editing \command{divacalc} and use \texttt{solver=2}. This can be usefull if you do not calculate error fields and do not use cross validation techniques but work with very fine grids. In this case execution time can be reduced. There are some tuning parameters which can enhance convergence but to exploit them, please contact us.
