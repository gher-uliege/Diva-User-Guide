...
\begin{figure}[htpb]
	...
	\parbox{.5\textwidth}{
		...
		\caption[Schema of the gridding problem.]{Schema of the gridding problem: the blue dots indicate data positions, while the nodes of the grid  are the points where the field has to be determined.\label{gridproblem}}
		}
\end{figure}
...
\begin{figure}[htpb]
	...
	\parbox{.5\textwidth}{
		...
		\caption[Interpolation vs. approximation.]{Interpolation provides a solution that goes across all the data points, while the approximation has only to be "close" to the measurements, but with a relative "smoothness".}
		}
\end{figure}
...
\caption[Kernel function.]{Kernel function is given in red, while the blue curve comes from the analysis of a single point.\label{kernel1}}
\end{figure}
