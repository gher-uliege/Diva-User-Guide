...
Perform the analysis with your "data". In the download section, you can download the topography in a NetCDF file \file{diva\_bath.nc}. Save this file into your \directory{input}. Then on the command line, placed in this \directory{input}, execute \command{../../bin/divabath2topogrd.a}. This will create \file{topo.grd} and \file{TopoInfo.dat}.
...
\command{divabin} performs a binning of the data with the spatial resolution defined in \file{param.par}. The script is called by \command{divafit} and/or by \command{diva3Dsnop} if the parameter ``binning'' is set to the value ``1'' in these scripts (at the beginning). The purpose of this optional binning is to improve the quality of the parameters (CL and/or SNR) optimization by removing a part of the highly correlated observations (taken by a boat in a restricted area, for instance).  
