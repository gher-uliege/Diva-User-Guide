...
The GEBCO bathymetry is distributed by the British Oceanographic Data Centre (BODC). If you have already registered, you can log in (https://www.bodc.ac.uk/my\_account/login/), otherwise it is necessary to register to become a new user. 

Download the complete, global GEBCO One-Minute Grid file (\file{90n90s180w180e.zip}) from \url{http://www.bodc.ac.uk/data/online\_delivery/gebco/} and unzip it to obtain the file \file{GridOne.grd}. Download the software \texttt{GebcoCE\_GridOnly} as well and run it. 
...
\subsection{Method 2: interpolation of individual topography measurements\label{sec:topotopex}}
%-------------------------------------------------------------------------
...
Goto \url{http://gher-diva.phys.ulg.ac.be/web-vis-test/diva.html}, upload any data (it can be the example provided on the web).
...
Then perform the analysis with your "data". In the download section, you can download the topography in a netcdf file \file{diva\_bath.nc}. Save this file into your \directory{input}. Then on the command line, placed in this \directory{input}, execute \command{../../bin/divabath2topogrd.a}. This will create \file{topo.grd} and \file{TopoInfo.dat}.
...
\centering
\parbox{.5\textwidth}{
\includegraphics[width=.45\textwidth,bb=0 0 488 254]{divaproblems}
...
\textbf{Note:} when using advection \index{Advection} constraint and variable $L$, \command{divafit} will not provide a very meaningful value.
